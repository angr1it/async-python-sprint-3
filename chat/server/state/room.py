...
from chat.manage_files import write_file, read_file
...
class RoomStore:
    ...
    def get_rooms_by_user(self, user: User) -> list[Room]:
        try:
            ...
            rooms = [
                ...
                if self.store[key].deleted is False
            ]
    ...
    def add_user_to_room(
        self, room: Room, admin: User, new_user: User
    ) -> bool:
        if room.room_type == RoomType.private:
            return False
    ...
    def remove_user_from_room(
        self, room: Room, admin: User, remove_user: User
    ) -> bool:
        if room.room_type == RoomType.private:
            return False
    ...
    def leave(self, user: User, room: Room) -> bool:
        ...
        if room.key not in self.user_rooms[user.username]:
            return False
        ...
        if (
            len(room.admins) == 1 and self.user_is_admin(room=room, user=user)
        ) or room.room_type == RoomType.private:
    ...
    def __users_to_dict(self):
        ...
        for key, value in self.user_rooms.items():
            data[key] = json.dumps(
                [str(key_room) for key_room in value], indent=4
            )
    ...
    def __load_user_room(self, data: dict):
        for name, user_data in data.items():
            ...
            self.user_rooms[name] = [
                uuid.UUID(room_key) for room_key in user_data_list
            ]
