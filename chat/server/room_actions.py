...
from chat.server.state.room import RoomStore, Room, RoomType
...
class JoinRoomAction(Command):
    @staticmethod
    def get_join_room_notification(
        user_name: str, room_name: str, success: bool, reason: str
    ):
        return UserAction(
    ...
    async def run(
        ...
        try:
            if RoomStore().join(
                ...
                room=RoomStore().get_room_by_name(room_str),
            ):
                await NotificationStore().process(
                    ws=ws_response,
                    notification=JoinRoomAction.get_join_room_notification(
...
class LeaveRoomAction(Command):
    @staticmethod
    def get_leave_room_notification(
        user_name: str, room_name: str, success: bool, reason: str
    ):
        return UserAction(
    ...
    async def run(
        ...
        if RoomStore().user_in_room(
            ...
            await NotificationStore().process(
                ws=ws_response,
                notification=LeaveRoomAction.get_leave_room_notification(
                    user_name=meta.user_name,
                    room_name=room_name,
                    success=True,
                    reason="",
...
class CreateRoomAction(Command):
    @staticmethod
    def get_create_room_notification(
        user_name: str, room_name: str, success: bool, reason: str
    ):
        return UserAction(
    ...
    async def run(
        ...
        if not meta.loggedin:
            await NotificationStore().process(
                ws=ws_response,
                notification=CreateRoomAction.get_create_room_notification(
                    user_name=meta.user_name,
                    room_name=room_name,
                    success=False,
                    reason=NO_AUTH,
...
class DeleteRoomAction(Command):
    @staticmethod
    def get_delete_room_notification(
        user_name: str, room_name: str, success: bool, reason: str
    ):
        return UserAction(
    ...
    async def run(
        ...
        try:
            ...
            await NotificationStore().process(
                ws=ws_response,
                notification=DeleteRoomAction.get_delete_room_notification(
                    user_name=meta.user_name,
                    room_name=room_name,
                    success=True,
                    reason="",
...
class AddUserAction(Command):
    ...
    def get_add_user_notification(
        ...
        return UserAction(
            ...
            reason=reason,
            user_name=user_name,
            payload={"room_name": room_name, "new_user": new_user},
        )
    ...
    async def run(
        ...
        try:
            if RoomStore().add_user_to_room(
                ...
                await NotificationStore().process(
                    ws=ws_response,
                    notification=AddUserAction.get_add_user_notification(
                        user_name=meta.user_name,
                        room_name=room_name,
                        new_user=new_user,
                        success=True,
...
class RemoveUserAction(Command):
    @staticmethod
    def get_remove_user_notification(
        user_name: str,
        room_name: str,
        remove_user: str,
        success: bool,
        ...
        return UserAction(
            ...
            reason=reason,
            user_name=user_name,
            payload={"room_name": room_name, "remove_user": remove_user},
        )
    ...
    async def run(
        ...
        try:
            ...
            await NotificationStore().process(
                ws=ws_response,
                notification=RemoveUserAction.get_remove_user_notification(
                    user_name=meta.user_name,
                    room_name=room_name,
...
class OpenDialogueAction(Command):
    @staticmethod
    def get_open_dialogue_notification(
        user_name: str, with_user: str, success: bool, reason: str
    ):
        return UserAction(
    ...
    async def run(
        ...
        try:
            ...
            await NotificationStore().process(
                ws=ws_response,
                notification=OpenDialogueAction.get_open_dialogue_notification(
                    user_name=meta.user_name,
                    with_user=with_user,
                    success=True,
                    reason="",
...
class DeleteDialogueAction(Command):
    ...
    async def run(
        ...
        try:
            ...
            RoomStore().delete_room(
                room=room, admin=UserStore().get_user(meta.user_name)
            )

            await NotificationStore().process(
                ws=ws_response,
                notification=DeleteDialogueAction.create_notification(
                    user_name=meta.user_name,
                    with_user=with_user,
                    success=True,
                    reason="",
