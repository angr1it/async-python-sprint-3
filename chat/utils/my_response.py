...
class WSResponse():
    def __init__(self, reader: StreamReader, writer: StreamWriter, chunk_size: int = CHUNK_SIZE, end_signal: bytes = END_SIGNAL) -> None:
    ...
    async def receive_bytes(self):
        ...
        while True:
            chunk = await self.reader.read(self.chunk_size + len(self.end_signal))
            data += chunk
    ...
    async def send_bytes(self, data: bytes):
        ...
        for i in range(0, int(len(data)/self.chunk_size) + 1):
            self.writer.write(data[i*self.chunk_size:(i+1)*self.chunk_size])
